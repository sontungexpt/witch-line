        ...
        if lval then
          ...
          if lval then
            ...
            if not lhl_name then -- never meet dynamic hl_name
              ...
              Statusline.set_side_value(indices, -1, lval, lhl_name or comp._left_hl_name,
        ...
        if rval then
          ...
          if rval then
            ...
            if not rhl_name then -- never meet dynamic hl_name
              ...
              Statusline.set_side_value(indices, 1, rval, rhl_name or comp._right_hl_name,
    ...
    if dep_id and not Manager.is_existed(dep_id) then
      ...
      if c then
        ...
        require("witch-line.utils.notifier").error(
          "The abstract component with id: " ..
          dep_id .. " is not registered. Please add it into abstract field in your config"
        )
      end
    end
  end
    ...
    if type(abstract) == "table" then
      for i = 1, #abstract do
        ...
        if type(c) == "table" and c.id then
          ...
          require("witch-line.utils.notifier").error(
            "Abstract component must be a component with an 'id' field: " .. tostring(c)
          )
        end
      end
    end
