...
is_marked = function(...)
    is_marked = require("witch-line.util.bitmask").is_marked
    return is_marked(...)
end
mark_bit = function(...)
    mark_bit = require("witch-line.util.bitmask").mark_bit
    return mark_bit(...)
end
...
--- @field min_screen_widths? CompId[]
--- @field win_individuals? CompId[]
...
setmetatable(Statusline, {
    __index = function(t, winid)
        if winid == 0 then
            return Statusline[api.nvim_get_current_win()]
        end

        if not win_closed_auid then
            win_closed_auid = api.nvim_create_autocmd("WinClosed", {
                callback = function(e)
                    rawset(Statusline, tonumber(e.match), nil)
                end,
            })
        end
...
local function get_flex_sorted(statusline)
    ...
    local sorted, n = {}, 0
    local comps, slots = statusline.states, statusline.slots
    for i = 1, #slots do
...
local function compute_statusline_width(win_state)
    local total, slots, comps = 0, win_state.slots, win_state.states
    for i = 1, #slots do
...
M.render = function(winid)
    ...
    nvim_set_option_value("statusline", build_value(statusline.slots, comp_state, hidden_slots), { win = winid })
end
...
M.inspect = function(winid)
    require("witch-line.util.notifier").info(vim.inspect(winid and Statusline[winid] or Statusline))
end
