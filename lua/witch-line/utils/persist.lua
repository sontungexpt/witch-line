...
local function serialize_tbl_funcs_rec(tbl, seen)
	local t = type(tbl)
	if t == "function" then
		return string.dump(tbl)
	elseif t ~= "table" then
		return tbl
	end
	seen = seen or {}
	...
	for k, v in pairs(tbl) do
		...
		elseif type(v) == "function" then
			...
			tbl[k] = serialize_tbl_funcs_rec(v, seen)
...
function M.serialize_function(value)
	local t = type(value)
	if t == "function" then
		return string.dump(value)
	elseif t == "table" then
		return serialize_tbl_funcs_rec(value, {})
	end
...
M.serialize_table = function(tbl, without_func, pretty)
	assert(type(tbl) == "table")
...
function M.serialize_table_as_bytecode(tbl, without_func)
	local str = M.serialize_table(tbl, without_func)
